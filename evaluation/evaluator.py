...
class Evaluator:
    ...
    @abstractmethod
    def evaluate(
        self, embeddings: Union[str, Dict[str, np.ndarray]], **kwargs
    ) -> Dict[str, float]:
        pass
...
class SupervisedEvaluator(Evaluator):
    def __init__(
        ...
        super(SupervisedEvaluator, self).__init__(
            name, meta_dataset, SimpleDataset, model, batch_size, fields
        )
    ...
    def evaluate(self, embeddings, **kwargs):
        ...
        if type(self.test_dataset) == str and os.path.isdir(self.test_dataset):
            split_dataset = datasets.load_dataset(
                "csv",
                data_files={
                    "train": f"{self.test_dataset}/train.csv",
        ...
        logger.info(
            f"Loaded {len(split_dataset['train'])} training and {len(split_dataset['test'])} test documents"
        )
        ...
        eval_fn = (
            self.regression if self.task == SupervisedTask.REGRESSION else self.classify
        )
        ...
        if self.task == SupervisedTask.REGRESSION:
            indi_results = {
                split_dataset["test"]["paper_id"][i]: {
                    **{
                        metric: preds[i] - split_dataset["test"]["label"][i]
    ...
    @staticmethod
    def read_dataset(
        data: datasets.DatasetDict, embeddings: Dict[str, np.ndarray]
    ) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        train, test = data["train"], data["test"]
        x_train, x_test = np.array(
            [
                embeddings[str(paper["paper_id"])]
                for paper in train
                if str(paper["paper_id"]) in embeddings
            ]
        ), np.array(
            [
                embeddings[str(paper["paper_id"])]
        ...
        y_train, y_test = np.array(
            [paper["label"] for paper in train if str(paper["paper_id"]) in embeddings]
        ), np.array(
    ...
    def classify(
        ...
        if self.task == SupervisedTask.MULTILABEL_CLASSIFICATION:
            ...
            svm = GridSearchCV(
                estimator=estimator, cv=cv, param_grid={"C": Cs}, n_jobs=10
            )
    ...
    def regression(
        ...
        svm = GridSearchCV(
            estimator=svm, cv=cv, param_grid={"C": Cs}, verbose=1, n_jobs=n_jobs
        )
        svm.fit(x_train, y_train)
        preds = svm.predict(x_test)
        return preds
        ...
        if self.task == SupervisedTask.REGRESSION:
            for m in self.metrics:
                if m in SUPERVISED_TASK_METRICS[self.task]:
                    result = tuple(SUPERVISED_TASK_METRICS[self.task][m](test, preds))[
                        0
                    ]
                    if m != "mse":
            ...
            for m in self.metrics:
                ...
                if split_m[0] in SUPERVISED_TASK_METRICS[metric_task]:
                    if len(split_m) > 1:
                        result = SUPERVISED_TASK_METRICS[metric_task][split_m[0]](
                            test, preds, average=split_m[1]
                        )
                    else:
                        result = SUPERVISED_TASK_METRICS[metric_task][split_m[0]](
                            test, preds
...
class IREvaluator(Evaluator):
    def __init__(
        ...
        model: Model,
        metrics: tuple,
        dataset_class=IRDataset,
        batch_size: int = 16,
        fields: list = None,
        key=None,
    ):
        super(IREvaluator, self).__init__(
            name, meta_dataset, dataset_class, model, batch_size, fields, key
        )
        self.test_dataset = test_dataset
        self.metrics = metrics
    ...
    def evaluate(self, embeddings, **kwargs):
        ...
        if type(self.test_dataset) == str and os.path.isdir(self.test_dataset):
            split_dataset = datasets.load_dataset(
                "json", data_files={"test": f"{self.test_dataset}/test_qrel.jsonl"}
            )
        else:
            split_dataset = datasets.load_dataset(
                self.test_dataset[0], self.test_dataset[1]
    ...
    def retrieval(
        self, embeddings, qrels: Dict[str, Dict[str, int]]
        ...
        for qid in qrels:
            if qid in embeddings:
                ...
                cands = np.array(
                    [embeddings[cid] for cid in qrels[qid] if cid in embeddings]
                )
